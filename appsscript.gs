...
function webActivatePartySeal(params) {
  ...
  let row = _findRow(sheet, sealId, PARTY_COL.SEAL_ID);
  ...
  sheet.getRange(row, PARTY_COL.START).setNumberFormat("h:mm AM/PM");
  sheet.getRange(row, PARTY_COL.EXPIRY).setValue(expiry);
  sheet.getRange(row, PARTY_COL.EXPIRY).setNumberFormat("h:mm AM/PM");
  sheet.getRange(row, PARTY_COL.ROOM).setValue(room);
...
// ============================================================
//  RETURN PARTY SEAL
//  Clears room (column F) — mirrors what onEdit does on room clear
// ============================================================
function webReturnPartySeal(params) {
  ...
  const ss    = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName("Party Seals");
  const row   = _findRow(sheet, sealId, PARTY_COL.SEAL_ID);
  if (!row) throw new Error('Party seal not found: ' + sealId);

  // Clear room → onEdit would then clear the rest, but since this
  // is a direct write (not a user edit), we clear everything manually
  sheet.getRange(row, PARTY_COL.STATUS).clearContent();
  sheet.getRange(row, PARTY_COL.START).clearContent();
  sheet.getRange(row, PARTY_COL.EXPIRY).clearContent();
  sheet.getRange(row, PARTY_COL.ADD_TIME).clearContent();
  sheet.getRange(row, PARTY_COL.ROOM).clearContent();
